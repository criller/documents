...
关闭Elasticsearch：
```
[criller@cc7 elasticsearch-7.4.1]$ jps
4248 Elasticsearch
5180 Jps
...
ik_smart: 会做最粗粒度的拆分，已被分出的词语将不会被其他词语占用
```
PUT /lib3
{
    "settings":{
...
(5)cardinality:求基数(基数表示互不相同的个数，例如性别，男和女，那么基数就是2 )
...
对那些有唱歌兴趣的用户按年龄分组,并且获取每组的平均年龄，再根据平均年龄进行降序排序
```
GET /lib3/user/_search
{
  "query": {
